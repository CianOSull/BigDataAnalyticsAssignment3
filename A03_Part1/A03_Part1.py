...
def my_main(input_folder, output_file, year):
    # Aggregate the trips for each different birth_year.
    # Consider only trips where user_type is “Subscriber” and
    # birth_year bigger or equal than a given year.
    # For each valid birth_year compute the start_hour with most trips. Output the results by increasing birth_year.
    # Note: if a birth_year has a tie, with two or more start_hours having the very
    #  same most trips, then break the tie by selecting the smallest start_hour.
    ...
    for filename in data_filename:
        ...
        for line in data_file.readlines():
            ...
            if year <= attributes[13] and attributes[12] == "Subscriber":
                time = attributes[0].split(" ")[1].split(":")
                # print(time[0])
                # print(attributes[12])
                # Increase the count of the start station
                if attributes[13] in birth_dict:
                    # birth_dict[attributes[13]] += 1
                    # birth_dict[attributes[13]] += 1
                    # birth_dict[attributes[13]] = time_list[0] + 1
                    # time_list[int(time[0]) - 1] += 1
                    # birth_dict[attributes[13]] = time_list[int(time[0]) - 1]
                    # birth_dict[attributes[13]] = time_list
