...
def my_main(spark, my_dataset_dir, year):
    ...
    # Select all the needed rows and only select rows where birth year is corrrect and user type is correct
    selectDF = inputDF.select(inputDF['start_time'], inputDF['user_type'], inputDF['birth_year']).filter(inputDF["user_type"] == "Subscriber").filter(year <= inputDF['birth_year'])

    # Count the number of trips
    countDF = selectDF.groupBy(["birth_year"]).agg({"birth_year": "count"}).withColumnRenamed("birth_year", "birth")

    # Join the two dataframes
    joinDF = selectDF.join(countDF, selectDF["birth_year"] == countDF["birth"], "inner")

    dropDF = joinDF.drop("birth").drop("user_type")
